...
public class KlausurService {
    ...
    public KlausurService(KlausurRepository repo,
                          LsfValidierung lsfValidierung,
                          BuchungsValidierung buchungsValidierung) {
        this.repo = repo;
    ...
    public String klausurHinzufuegen(Klausur klausur) throws IOException {
        if (!buchungsValidierung.klausurLiegtImPraktikumsZeitraum(klausur)) {
            log.error("Klausurregistrierung", "Registrierungsversuch einer Klausur außerhalb des " +
                    "Praktikumszeitraums", LocalDateTime.now());
            return "Die Klausur liegt ausserhalb der Praktikumszeit. Gültig sind Mo. - Fr. im Zeitraum vom "
                    + buchungsValidierung.startTag + " bis " + buchungsValidierung.endTag + " zwischen "
                    + buchungsValidierung.startZeit + " und " + buchungsValidierung.endZeit + ".";
        }

        if (!lsfValidierung.gueltigeLsfId(klausur.getLsfId())) {
            log.error("Klausurregistrierung", "Registrierungsversuch einer Klausur mit ungültiger LSF-ID.",
                    LocalDateTime.now());
        ...
        if (klausurAusDB == null) {
            log.info("Klausurregistrierung", "Klausur mit LsfId " + klausur.getLsfId() + " registriert.",
                    LocalDateTime.now());
            repo.save(klausur);
        } else {
            log.error("Klausurregistrierung", "Klausur mit LsfId " + klausur.getLsfId()
                    + " existiert bereits.", LocalDateTime.now());
        }
        return "";
    }

    public Klausur findeKlausurMitLsfId(Long lsfId) {
        return repo.klausurMitLsfId(lsfId);
    }
